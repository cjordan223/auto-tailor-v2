...
RESUME_CHUNKS = [
    ...
    {"id": "SKILLS.Automation & Productivity",
        "editable": True, "type": "skill_line"},
    {"id": "SKILLS.Security & Operating Systems",
        "editable": True, "type": "skill_line"},
...
def reorder_resume_sections(content: str) -> str:
    ...
    header_match = re.search(
        r'(\\documentclass.*?(?=\\section|\\begin{document}))', content, re.DOTALL)
    if header_match:
        sections['header'] = header_match.group(1)

    # Find Summary section
    summary_match = re.search(
        r'(\\section\*?\{.*?(?:Summary|Professional Summary).*?\}.*?)(?=\\section|\Z)', content, re.DOTALL | re.IGNORECASE)
    if summary_match:
        sections['summary'] = summary_match.group(1)

    # Find Experience section
    exp_match = re.search(
        r'(\\section\*?\{.*?(?:Experience|Work Experience|Professional Experience).*?\}.*?)(?=\\section|\Z)', content, re.DOTALL | re.IGNORECASE)
    if exp_match:
        sections['experience'] = exp_match.group(1)

    # Find Skills section
    skills_match = re.search(
        r'(\\section\*?\{.*?(?:Skills|Technical Skills).*?\}.*?)(?=\\section|\Z)', content, re.DOTALL | re.IGNORECASE)
    if skills_match:
        sections['skills'] = skills_match.group(1)

    # Find Education section
    edu_match = re.search(
        r'(\\section\*?\{.*?(?:Education).*?\}.*?)(?=\\section|\Z)', content, re.DOTALL | re.IGNORECASE)
    if edu_match:
        sections['education'] = edu_match.group(1)

    # Find Certifications section
    cert_match = re.search(
        r'(\\section\*?\{.*?(?:Certifications?).*?\}.*?)(?=\\section|\Z)', content, re.DOTALL | re.IGNORECASE)
    if cert_match:
...
def add_resume_markers(content: str) -> str:
    ...
    header_end_match = re.search(
        r'(\\begin\{document\}.*?\\section\{PROFESSIONAL SUMMARY\})', content, re.DOTALL)
    ...
    summary_match = re.search(
        r'(\\section\{PROFESSIONAL SUMMARY\}.*?\\item \\small\{%)(.*?)(%\})', content, re.DOTALL)
    ...
    exp_match = re.search(
        r'(\\section\{WORK EXPERIENCE\}.*?)(?=\\section\{TECHNICAL SKILLS\})', content, re.DOTALL)
    if not exp_match:
        return content
    ...
    skills_match = re.search(
        r'(\\section\{TECHNICAL SKILLS\}.*?\\resumeSubHeadingListStart.*?\\item \\small\{)(.*?)(\}\s*\\resumeSubHeadingListEnd)', content, re.DOTALL)
    if not skills_match:
        return content
    ...
    edu_match = re.search(
        r'(\\section\{EDUCATION\}.*?)(?=\\section\{CERTIFICATIONS\})', content, re.DOTALL)
    if not edu_match:
        return content
    ...
    cert_match = re.search(
        r'(\\section\{CERTIFICATIONS\}.*?\\end\{document\})', content, re.DOTALL)
    if not cert_match:
        return content
    ...
    result.append("")
    result.append(summary_header + wrap_chunk(summary_text,
                  "RESUME.SUMMARY", True) + summary_end)
    result.append("")
...
def add_cover_letter_markers(content: str) -> str:
    ...
    header_match = re.search(
        r'(\\begin\{document\}.*?)(?=\\vspace\{16pt\}.*?\\begin\{flushright\})', content, re.DOTALL)
    if not header_match:
        return content
    ...
    date_match = re.search(
        r'(\\begin\{flushright\}.*?August.*?\\end\{flushright\})', content, re.DOTALL)
    if not date_match:
        return content
    ...
    body_match = re.search(
        r'\\end\{flushright\}(.*?)\\end\{document\}', content, re.DOTALL)
    if not body_match:
        return content
    ...
    result.append(wrap_chunk(
        "If your mandate is to build reliable, secure, and scalable software that solves complex problems, I would like to help you ship that.", "COVER.P4", True))
    result.append("")
    result.append("\\vspace{16pt}")
    result.append("")
...
def init_files(quiet: bool = False) -> None:
    ...
    resume_input_local = Path(
        "templates/Baseline_Resume/Conner_Jordan_Software_Engineer copy.tex")
    resume_input_original = Path(
        "~/Doc/Sandbox_v3/Baseline_Resume/Conner_Jordan_Software_Engineer copy.tex").expanduser()
    resume_output = Path(
        "templates/Baseline_Resume/Conner_Jordan_Software_Engineer llm_ready.tex")

    cover_input_local = Path(
        "templates/Basline_Cover_Letter/Conner_Jordan_Cover_Letter copy.tex")
    cover_input_original = Path(
        "~/Doc/Sandbox_v3/Basline_Cover_Letter/Conner_Jordan_Cover_Letter copy.tex").expanduser()
    cover_output = Path(
        "templates/Basline_Cover_Letter/Conner_Jordan_Cover_Letter llm_ready.tex")
    ...
    if cover_input.exists():
        ...
        if not quiet:
            print(
                f"Warning: Cover letter source file not found: {cover_input}")
