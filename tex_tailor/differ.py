...
def get_chunk_content(filepath: str, chunk_id: str) -> str:
    ...
    try:
        ...
        chunk_content = content[start_match.end(
        ...
        if status == 'equal':
            ...
            if COLORS_AVAILABLE:
                formatted_parts.append(
                    f"{Fore.RED}{Style.BRIGHT} -{token}- {Style.RESET_ALL}")
            else:
...
def create_diff_summary(diffs: List[str]) -> str:
    ...
    # Categorize changes
    resume_changes = [
        c for c in changed_chunks if c.startswith(('RESUME.', 'SKILLS.'))]
...
def show_diffs(quiet: bool = False) -> None:
    ...
    # Standard file paths
    original_resume = "templates/Baseline_Resume/Conner_Jordan_Software_Engineer llm_ready.tex"
    original_cover = "templates/Basline_Cover_Letter/Conner_Jordan_Cover_Letter llm_ready.tex"
    ...
    # Generate diffs
    diffs = get_all_chunk_diffs(
        original_resume, original_cover, tuned_resume, tuned_cover)
...
def export_diff_report(output_file: str) -> None:
    ...
    try:
        ...
        if not tuned_resume or not tuned_cover:
            raise RuntimeError(
                "No tuned files found in out/. Run 'tex-tailor apply' first.")
