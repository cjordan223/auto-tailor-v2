...
7.  **SKILLS - NEVER REMOVE, ONLY ADD, NO DUPLICATES:** For ATS (Applicant Tracking System) visibility, you must NEVER remove existing skills. Keep ALL existing skills from the baseline. ONLY add new skills from the job description if they are directly supported by the candidate's experience AND the skill does not already exist in ANY skills category. Before adding a skill, check ALL skill categories to prevent duplicates. If a skill already exists anywhere in the skills section, do not add it again. If a skill is mentioned in the JD but is not evidenced in the resume, add it to "suggested_additions" instead. When in doubt, return `null` to keep skills unchanged.
...
-   **Cover Letter Paragraph 2 (Body):** Connect a key job requirement to a specific achievement with rich detail. "The job description emphasizes a need for expertise in healthcare data analysis. In my previous role at [Previous Company], I led the development of predictive models for patient outcomes, working closely with clinical teams to understand their workflow needs and data requirements. This collaborative approach, combined with my technical expertise in machine learning algorithms, resulted in a 25% improvement in treatment recommendations and significantly reduced the time clinicians spent on manual analysis. This experience has prepared me to contribute effectively to HealthMetric Analytics' clinical insights team, where I can apply both my technical skills and my understanding of healthcare workflows."
-   **Cover Letter Paragraph 3 (Impact & Fit):** Elaborate on how the candidate's background creates value. "Beyond technical skills, my experience in cross-functional collaboration has taught me the importance of translating complex technical concepts into actionable insights for stakeholders. At [Previous Company], I regularly presented findings to both technical and non-technical audiences, ensuring that our data-driven solutions were accessible and actionable. This ability to bridge the gap between technical implementation and business value would be particularly valuable at HealthMetric Analytics, where clear communication between data science and clinical teams is crucial for success."
-   **Skills (NEVER REMOVE, NO DUPLICATES):** Keep ALL existing skills. Only ADD new skills if they are in the JD AND supported by the resume content AND not already listed in ANY category. For example, if "Cloud & DevOps" has "AWS, Docker, Kubernetes, Terraform" and the JD requires "Terraform" for a security role, do NOT add "Terraform" to "Security & Operating Systems" because it already exists in Cloud & DevOps. Return `null` for that category. Another example: if the JD requires "Linux" and the resume mentions Linux experience, check ALL categories first - if Linux is not found anywhere, then you can add it to the most appropriate category. NEVER return just "AWS, Terraform" when the original was "AWS, Docker, Kubernetes" - that removes Docker and Kubernetes which hurts ATS visibility. If the resume has no evidence of a skill, add it to `suggested_additions` instead.
